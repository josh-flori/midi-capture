...
def find_note_positions(video_file):
    ...
    while success and count < number_of_frames:
        ...
        cv2.imwrite("/users/josh.flori/project/0/frame{}.jpg".format(count), final_img)
...
start_list, end_list, bottom_of_note, final_img = find_note_positions('second.mp4')
...
for note in range(0,15): ## loops from 0-19
    note_number = note+38
    silenced, destination, upmotion, downmotion, error_log, silenced_contenders, destination_contenders, upper_lower_accumulation, note_height  = get_parameters2('second.mp4', index_list[note], window_height)
    if len(error_log) > 10:
        print('POTENTIAL ERROR FOUND')
    print(silenced, upmotion, destination, downmotion)
    print(index_list[note])
    temp_string,debug_list = get_midi_information('second.mp4', index_list[note], 33, silenced, upmotion, destination, downmotion, note_number, note)
