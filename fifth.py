...
start_list, end_list, bottom_of_note, final_img = find_note_positions('fifth.mp4')
...
file = open('fifth.txt','w') 
file.write(str(midi_string)+'])') 
file.close()


filenames = ['first.txt', 'second.txt', 'third.txt', 'fourth.txt','fifth.txt']
with open('/users/josh.flori/output.py', 'w') as outfile:
    outfile.write('import midi\n')
    for fname in filenames:
        with open(fname) as infile:
                outfile.write(infile.read())
    outfile.write('\n')   
    outfile.write('midi.write_midifile("my_song.mid", my_song)') 
    outfile.close()
