...
start_list, end_list, bottom_of_note, final_img = find_note_positions('fourth.mp4')
...
for note in range(0,19): ## loops from 0-19
    note_number = note+71
    silenced, destination, upmotion, downmotion, error_log, silenced_contenders, destination_contenders, upper_lower_accumulation, note_height  = get_parameters2('fourth.mp4', index_list[note], window_height)
    if len(error_log) > 10:
        print('POTENTIAL ERROR FOUND')
    print(silenced, upmotion, destination, downmotion)
    print(index_list[note])
    temp_string,debug_list = get_midi_information('fourth.mp4', index_list[note], 33, silenced, upmotion, destination, downmotion, note_number, note)
    debug_dict[note]=debug_list
    midi_string = midi_string + temp_string

    
os.chdir('/Users/josh.flori')
file = open('fourth.txt','w') 
