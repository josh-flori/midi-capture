...
192.168.2.9 = 17 notes .... note+21
192.168.2.8 = 15 notes .... note+38
192.168.2.7 = 18 notes .... note+53
192.168.2.6 = 19 notes .... note+71
192.168.2.5 = 19 notes .... note+90
...
def find_increment(start_list):
    increment=.45/(len(start_list)/2)
    return increment #mind you this won't work if you've imported division as that leaves decimals and you can't get a pixel decimal value
...
def get_slice_values(start_list, end_list, bottom_of_note):
    ...
    new_start_list=[]
    increment=find_increment(start_list)
    for i in range(0,len(start_list)): # so this section is to account for the fact that the camera lense is curved and a note on the left will actually move to the left during it's motion, a note in the middle will stay in place horizontally and a note on right will move to the right. this gets subranges that range from left to right within the note
        difference=end_list[i]-start_list[i]
        new_start_list.append(start_list[i]+int((difference*increment*(i+1))))
    index_list =[]
    ...
    for i in range(0,len(start_list)): #start_list is arbitrary; all lists have the same length so we could have used any of them
        index_list.append((slice(bottom_of_note[i]-65, bottom_of_note[i]+10), slice(new_start_list[i], new_start_list[i]+1))) # this now sets the index range to be a single pixel wide. EDGYYYYY AF WTH OMGGG
        ...
        try:
            ...
            if np.sum(final_img) < final_img.size * 200: #this will make sure the code only triggers once the lights turn on since a black image, once converted, == 1530000. i've set the value a little lower just to allow for some error
                for pixel_row in range(0,window_height):  ## loop over all of the rows in the image, or window, for the note
                    if np.sum(final_img[pixel_row,:]) == 0:
        ...
        try: ## the reason we need this is because for whatever reason "vidcap.read" which output a false signal at the very end of the video once it fails to find a new frame, so we just need to avoid that error.
            ...
            if np.sum(final_img) < final_img.size * 200: #this will make sure the code only triggers once the lights turn on since a black image, once converted, == 1530000. i've set the value a little lower just to allow for some error
                if current_position == 'silenced':
                    if np.sum(final_img[upmotion,:]) == 0: ## "if note has started moving upward".... 77 marks the first movement row from bottom, grey background = 200, black note = 0 with some error, so <10 means the note has reached that position in space and therefore is in motion.
                    ...
                    if np.sum(final_img[destination,:]) == 0: ## "if note has reached destination".... 42 marks the first row needed to be reached for the note to be activated, or reach it's "destination" 
                    ...
                    elif np.sum(final_img[silenced,:])  == 0:  ## "if note went back down"
                        current_position = 'silenced'
                        velocity_count=0
                        tick = tick+3
                        debug_list.append('{}_silenced'.format(count))
                    ...
                    if np.sum(final_img[downmotion,:]) == 0:  ## if note has started moving downward
                        tick = tick+3
                    ...
                    if np.sum(final_img[silenced,:]) == 0:  ## "if note has reached the bottom and is now silenced" where row 83 = the bottom
                        tick = tick+3
                        current_position = 'silenced'
...
temp_string=''
midi_string ='midi.Pattern(tracks=['
## i can't remember what this does
debug_dict={}
### get the note positions
start_list, end_list, bottom_of_note, final_img = find_note_positions('first.mp4')
...
for note in range(0,19): ## loops from 0-19
    note_number = note+70
    silenced, destination, upmotion, downmotion, error_log, silenced_contenders, destination_contenders, upper_lower_accumulation, note_height  = get_parameters2('fifth.mp4', index_list[note], window_height)
    if len(error_log) > 10:
        print('POTENTIAL ERROR FOUND')
    print(silenced, upmotion, destination, downmotion)
    print(index_list[note])
    temp_string,debug_list = get_midi_information('fifth.mp4', index_list[note], 33, silenced, upmotion, destination, downmotion, note_number, note)
    debug_dict[note]=debug_list
    midi_string = midi_string+temp_string
debug_df = pd.DataFrame.from_dict(debug_dict)
debug_df.to_csv('/Users/josh.flori/project/debug_results.csv')  
    
midi_string = midi_string[:len(midi_string)-1]
os.chdir('/Users/josh.flori')
file = open('goodtest.py','w') 
file.write('import midi\n')
file.write('my_song = '+str(midi_string)+'])\n') 
file.write('midi.write_midifile("my_song.mid", my_song)\n') 
file.close()
