...
for note in range(0,17): ## loops from 0-19
    note_number = note+21
    silenced, destination, upmotion, downmotion, error_log, silenced_contenders, destination_contenders, upper_lower_accumulation, note_height  = get_parameters2('first.mp4', index_list[note], window_height)
    if len(error_log) > 10:
        print('POTENTIAL ERROR FOUND')
    print(silenced, upmotion, destination, downmotion)
    print(index_list[note])
    temp_string,debug_list = get_midi_information('first.mp4', index_list[note], 33, silenced, upmotion, destination, downmotion, note_number, note)
